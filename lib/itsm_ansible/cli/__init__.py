...
class CLI(object):
    ...
    def __init__(self, args, callback=None):
        ...
        # add by ganjl for disable host key check
        C.HOST_KEY_CHECKING = False
        # C.DEFAULT_CALLBACK_PLUGIN_PATH+=":"+ os.path.abspath(os.path.split(os.path.realpath(__file__))[0]+"/../plugins/callback")
    ...
    def base_parser(usage="", output_opts=False, runas_opts=False, meta_opts=False, runtask_opts=False,
                    vault_opts=False, module_opts=False,
                    async_opts=False, connect_opts=False, subset_opts=False, check_opts=False, inventory_opts=False,
        ...
        if vault_opts:
            parser.add_option('--ask-vault-pass', default=C.DEFAULT_ASK_VAULT_PASS, dest='ask_vault_pass',
                              action='store_true',
            ...
            parser.add_option('--new-vault-password-file', dest='new_vault_password_file',
                              help="new vault password file for rekey", action="callback", callback=CLI.expand_tilde,
                              type=str)
        ...
        if runas_opts or runas_prompt_opts:
            parser.add_option('-K', '--ask-sudo-pass', default=C.DEFAULT_ASK_SUDO_PASS, dest='ask_sudo_pass',
                              action='store_true',
        ...
        if connect_opts:
            ...
            parser.add_option('--private-key', '--key-file', default=C.DEFAULT_PRIVATE_KEY_FILE,
        ...
        if meta_opts:
            parser.add_option('--force-handlers', default=C.DEFAULT_FORCE_HANDLERS, dest='force_handlers',
                              action='store_true',
