...
from itsm_ansible.inventory.script import InventoryScript
...
class Inventory(object):
    ...
    def parse_inventory(self, host_list):
        ...
        self.parser = InventoryScript(loader=self._loader, groups=self.groups, filename=host_list)
    ...
    def _get_group_variables(self, groupname, vault_password=None):
        ...
        vars_results = [plugin.get_group_vars(group, vault_password=vault_password) for plugin in self._vars_plugins if
                        hasattr(plugin, 'get_group_vars')]
    ...
    def _get_host_variables(self, hostname, vault_password=None):
        ...
        vars_results = [plugin.run(host, vault_password=vault_password) for plugin in self._vars_plugins if
                        hasattr(plugin, 'run')]
        for updated in vars_results:
            if updated is not None:
                vars = combine_vars(vars, updated)
        ...
        vars_results = [plugin.get_host_vars(host, vault_password=vault_password) for plugin in self._vars_plugins if
                        hasattr(plugin, 'get_host_vars')]
        for updated in vars_results:
            if updated is not None:
                vars = combine_vars(vars, updated)
        ...
        for basedir in basedirs:
            ...
            if group and host is None:
                ...
                base_path = os.path.realpath(
                    os.path.join(to_unicode(basedir, errors='strict'), "group_vars/%s" % group.name))
                results = combine_vars(results, self._variable_manager.add_group_vars_file(base_path, self._loader))
            elif host and group is None:
                # same for hostvars in dir/host_vars/name_of_host
                base_path = os.path.realpath(
                    os.path.join(to_unicode(basedir, errors='strict'), "host_vars/%s" % host.name))
