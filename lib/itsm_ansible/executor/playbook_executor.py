...
from itsm_ansible.playbook import Playbook
from ansible.template import Templar
from itsm_ansible.plugins.callback.itsm_ansible_log import CallbackModule
...
class PlaybookExecutor:
    ...
    def __init__(self, playbooks, inventory, variable_manager, loader, options, passwords, pt, task_id):
        ...
        self._pt = pt
        callback = CallbackModule(task_id=task_id)
        ...
        try:
            for playbook_path in self._playbooks:
                ...
                for play in plays:
                    ...
                    if play.vars_prompt:
                        for var in play.vars_prompt:
                            ...
                            if vname not in self._variable_manager.extra_vars:
                                if self._tqm:
                                    self._tqm.send_callback('v2_playbook_on_vars_prompt', vname, private, prompt,
                                                            encrypt, confirm, salt_size, salt, default)
                    ...
                    if self._tqm is None:
                        ...
                        for batch in self._get_serialized_batches(new_play):
                            ...
                            if new_play.max_fail_percentage is not None and \
                                            int((new_play.max_fail_percentage) / 100.0 * len(batch)) > int(
                                                        (len(batch) - failed_hosts_count) / len(batch) * 100.0):
                                break
                    ...
                    i = i + 1  # per play
                    self._tqm.send_callback('v2_playbook_on_finish', pb)
